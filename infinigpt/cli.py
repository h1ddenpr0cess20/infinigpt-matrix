...
def build_parser() -> argparse.ArgumentParser:
    ...
    Returns:
        Configured `argparse.ArgumentParser` for the InfiniGPT CLI.
    """
    parser = argparse.ArgumentParser(
        prog="infinigpt-matrix",
        description=(
            "InfiniGPT Matrix bot. Modern CLI that validates config and runs the app."
        ),
        add_help=True,
    )
    parser.add_argument(
        "-L",
        "--log-level",
        default=os.getenv("INFINIGPT_LOG_LEVEL", "INFO"),
    ...
    parser.add_argument("-u", "--openai-url", help="Override OpenAI API URL")
    parser.add_argument(
        "-S",
        "--server-models",
        dest="server_models",
        action="store_true",
        help="Fetch available models from the OpenAI server",
    )
...
def main(argv: Optional[List[str]] = None) -> int:
    """Entry point for the `infinigpt-matrix` CLI.
    ...
    overrides = {}
    if args.openai_url:
        overrides.setdefault("openai", {})["api_url"] = args.openai_url
    if args.model:
        overrides.setdefault("openai", {})["default_model"] = args.model
    if args.store_path:
    ...
    if args.verbose_mode:
        overrides.setdefault("openai", {})["verbose"] = True
    ...
    if args.server_models:
        try:
            ...
            client = OpenAIClient(base_url=cfg.openai.api_url.rsplit("/", 1)[0], timeout=cfg.openai.timeout)
            models = client.list_models()
            if models:
                cfg.openai.models = models
                if cfg.openai.default_model not in models and cfg.openai.default_model not in set(models.values()):
                    cfg.openai.default_model = next(iter(sorted(models.keys())))
