...
async def handle_ai(ctx: Any, room_id: str, sender_id: str, sender_display: str, args: str) -> None:
    ...
    Args:
        ctx: Application context providing `history`, `openai`, `matrix`,
    ...
    try:
        if getattr(ctx, "tools_enabled", False):
            ...
            data = await ctx.to_thread(openai.chat, messages=messages, model=ctx.model, options=ctx.options, timeout=ctx.timeout)
