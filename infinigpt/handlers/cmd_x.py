...
async def handle_x(ctx: Any, room_id: str, sender_id: str, sender_display: str, args: str) -> None:
    ...
    Args:
        ctx: Application context with `history`, `matrix`, `openai`, `model`,
    ...
    try:
        data = await ctx.to_thread(
            ctx.openai.chat, messages=ctx.history.get(room_id, target_user), model=ctx.model, options=ctx.options, timeout=ctx.timeout
        )
