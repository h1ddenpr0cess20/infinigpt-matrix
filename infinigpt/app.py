...
class AppContext:
    ...
    def __init__(self, cfg: AppConfig, executor: Optional[ThreadPoolExecutor] = None) -> None:
        self.cfg = cfg
        self.executor = executor or ThreadPoolExecutor(max_workers=4, thread_name_prefix="openai")
        ...
        self.openai = OpenAIClient(base_url=cfg.openai.api_url.rsplit("/", 1)[0], timeout=cfg.openai.timeout)
        # Support optional third prompt element used as a brevity clause
        _prompt = list(cfg.openai.prompt or ["you are ", "."])
        ...
        self.history = HistoryStore(
            ...
            personality=cfg.openai.personality,
            prompt_suffix_extra=_extra,
            max_items=cfg.openai.history_size,
        )
        # Expose commonly used fields
        self.models = cfg.openai.models
        self.default_model = cfg.openai.default_model
        self.model = cfg.openai.default_model
        self.default_personality = cfg.openai.personality
        self.personality = cfg.openai.personality
        self.options = cfg.openai.options
        self.timeout = cfg.openai.timeout
        self.admins = cfg.matrix.admins
        self.bot_id = "InfiniGPT"
        # Verbose mode: when True, omit brevity clause for new conversations
        try:
            self.verbose: bool = bool(getattr(cfg.openai, "verbose", False))
        except Exception:
            self.verbose = False
        try:
        ...
        if cfg.openai.mcp_servers:
            self.logger.info("MCP servers configured: %s", list(cfg.openai.mcp_servers.keys()))
            successful: Dict[str, Any] = {}
            for name, cfg_spec in cfg.openai.mcp_servers.items():
                if not cfg_spec:
                    continue
                try:
    ...
    def respond_with_tools(self, messages: List[Dict[str, Any]], *, tool_choice: str | None = "auto") -> str:
        log = getattr(self, "logger", logging.getLogger(__name__))
        try:
            result = self.openai.chat_with_tools(
                model=self.model,
                messages=messages,
